...
router.post("/add-new-user", async (req, res) => {
  ...
  pool.query(
    ...
    function (error, results, fields) {
      ...
      res
        .status(201)
        .json({ success: true, results, msg: "User created successfully" });
    }
  );
});
...
router.get("/get-user-details", function (req, res) {
  ...
  pool.query(sql, username, function (error, results, fields) {
    ...
    res
      .status(201)
      .json({ success: true, results, msg: "User retrieved successfully" });
  });
});
...
router.get("/get-all-user-details", function (req, res) {
  ...
  pool.query(sql, function (error, results, fields) {
    ...
    res.status(201).json({
      success: true,
      results,
      msg: "Users information retrieved successfully",
    });
  });
});
...
router.put("/edit-user-details", async (req, res) => {
  ...
  pool.query(
    `SELECT COUNT(*) as count FROM users WHERE name = '${requestPayload.name}';`,
    (error, result) => {
      if (error) {
